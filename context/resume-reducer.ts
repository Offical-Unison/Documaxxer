import type { Award, Certification, Education, Experience, Language, OptionalSectionKey, PersonalDetails, Project, ResumeData, ResumeState, VolunteerExperience } from "@/types/resume";

export const initialResumeData: ResumeData = {
  personalDetails: { firstName: "", lastName: "", headline: "", contact: { email: "", phone: "", location: "", website: "", linkedin: "", github: "" } },
  professionalSummary: "", experiences: [], education: [], skillGroups: [], projects: [], certifications: [], languages: [], interests: [], awards: [], publications: [], volunteerExperiences: [], references: [], customSections: [], optionalSections: [],
};
...
export function resumeReducer(state: ResumeState, action: ResumeAction): ResumeState {
  switch (action.type) {
    ...
    case "SET_SKILLS": return { ...state, resume: { ...state.resume, skillGroups: [{ id: "primary", name: "Skills", skills: action.payload }] } };
    ...
    case "SET_INTERESTS": return { ...state, resume: { ...state.resume, interests: action.payload } };
